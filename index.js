...
app.post('/api/loginadmin', async(req, res) => {
    try {
        ...
        if (user) {
            ...
            saveData(token);
            res.status(200).json({ message: "autorizado", token, loja, nome, Estabelecimento, codigo, telefone, email, cnpj });
        } else {
...
app.post('/lojas/add/categoria/:estabelecimento/', async(req, res) => {
    try {
        const formData = req.body; // Dados enviados pelo cliente
        const codigo = req.params.estabelecimento
            // Conectar ao MongoDB
...
app.get('/lojas/list/categorias/:estabelecimento', async(req, res) => {
...
app.get('/lojas/list/produtos/:estabelecimento', async(req, res) => {
...
app.delete('/lojas/excluir/categorias/:estabelecimento/:categoria', async(req, res) => {
    try {
        const { estabelecimento, categoria } = req.params;
        const client = new MongoClient(uri);
        await client.connect();
        const collection = client.db("Lojas").collection('Categorias' + estabelecimento);
...
app.post('/menugenius/add/imagem', upload.single('file'), async(req, res) => {
    ...
    // Endpoint para receber o upload da imagem
app.post('/lojas/add/imagem', upload.single('file'), async(req, res) => {
    try {
        if (!req.file) {
            res.status(400).json({ error: 'Nenhum arquivo enviado.' });
            return;
        ...
        blobStream.on('finish', async() => {
            try {
                // Configuração da URL de download da imagem (expira em 30 dias)
                const expires = new Date();
                expires.setDate(expires.getDate() + 30); // Expira em 30 dias

                const config = {
                    action: 'read',
                    expires: expires.toISOString(), // Define a data de expiração
                };
...
app.post('/lojas/add/produto/:estabelecimento/', async(req, res) => {
    try {
        const formData = req.body; // Dados enviados pelo cliente
        const codigo = req.params.estabelecimento
            // Conectar ao MongoDB
...
app.delete('/lojas/excluir/produtos/:estabelecimento/:id', async(req, res) => {
    try {
        const idDoProdutoParaApagar = req.params.id;
        const estabelecimento = req.params.estabelecimento;
        ...
        await client.connect();
        const collection = client.db("Lojas").collection('Produtos' + estabelecimento);
...
app.get('/menugenius/list/pedidos-geral/:estabelecimento', async(req, res) => {
    try {
        const codigo = req.params.estabelecimento;
...
app.get('/menugenius/list/pedidos/:estabelecimento', async(req, res) => {
    try {
        const codigo = req.params.estabelecimento;
        const status = req.query.status || 'Aberto'; // Padrão para 'Disponível' se não especificado
        ...
        // Selecionar a coleção de Pedidos e buscar os dados filtrando pelo status
        const categorias = await db.collection('Pedidos' + codigo).find({ status }).toArray();
...
app.get('/pedidos', (req, res) => {
    // Simule os dados que você deseja enviar para o cliente
    const dados = { estabelecimento: 1234, mensagem: 'Mensagem do servidor' };
    res.json(dados);
});
